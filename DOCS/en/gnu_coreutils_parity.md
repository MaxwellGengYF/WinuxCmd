...
| `ls` | [ls invocation](https://www.gnu.org/software/coreutils/manual/html_node/ls-invocation.html) | common display flags plus `-B`, `-F`, `-p`, `-Q`, `-q`, and `-R` now align better | remaining gaps: `-b`, `-L`, `-I`, `-U`, `-X`, `-v`, `--time-style` |
...
| `sort` | [sort invocation](https://www.gnu.org/software/coreutils/manual/html_node/sort-invocation.html) | basic sorting exists, plus `-V` version sort | `-k`, `-t`, `-S`, `-R`, `-o` |
| `uniq` | [uniq invocation](https://www.gnu.org/software/coreutils/manual/html_node/uniq-invocation.html) | basic dedupe exists | `-D`, `--group`, `--all-repeated`, `-f`, `-s`, `-w`, `-z` |
