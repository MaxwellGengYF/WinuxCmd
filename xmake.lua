...
local SET_PCXX_HEADER = false

function winux_cmd_set_pcxxheader(header)
    if SET_PCXX_HEADER then
...
target("winuxcmd-core")
    ...
    winux_cmd_set_pcxxheader("src/pch/pch.h")
...
if has_config("build_ffi") then
    target("winuxcore")
        add_rules("lc_basic_settings", {project_kind = "shared", enable_exception = true})
