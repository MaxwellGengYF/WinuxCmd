...
constexpr auto COLOR_RESET = L"\033[0m";    ///< Reset all attributes
constexpr auto COLOR_DIR = L"\033[01;34m";  ///< Directories (bold blue)
constexpr auto COLOR_EXEC =
    L"\033[01;32m";  ///< Executables (bold green)
constexpr auto COLOR_LINK =
    L"\033[01;36m";                             ///< Symbolic links (bold cyan)
constexpr auto COLOR_FILE = L"\033[0m";  ///< Regular files (default)
constexpr auto COLOR_ARCHIVE =
    L"\033[01;31m";  ///< Archives: .zip, .tar, .7z (bold red)
constexpr auto COLOR_SCRIPT =
    L"\033[01;33m";  ///< Scripts: .ps1, .sh, .py (bold yellow)
constexpr auto COLOR_SOURCE =
...
inline void set_output_capture_handles(HANDLE stdout_handle,
...
inline void invalidateCachedHandles() {
  g_handles_valid = false;
  g_console_checked = false;
}

inline bool is_stdout_pipe_closed() { return g_stdout_pipe_closed; }

inline bool is_stderr_pipe_closed() { return g_stderr_pipe_closed; }

inline void clear_pipe_closed_flags() {
...
inline bool writeConsole(const std::wstring_view& wstr) {
...
inline bool writeErrorConsole(const std::wstring_view& wstr) {
...
inline bool isOutputConsole() { return isConsoleHandle(getStdOut()); }

inline bool isErrorConsole() { return isConsoleHandle(getStdErr()); }

/**
 * @brief Check if terminal supports color
 * @return true if terminal supports color, false otherwise
 */
inline bool isTerminalSupportsColor() {
 ...
 */
inline bool setupConsoleForUnicode() {
...
// ============================================================================
namespace detail {
inline bool writeConsoleW(HANDLE h, const wchar_t* data, size_t len) {
...
inline bool writeFile(HANDLE h, const char* data, size_t len) {
  if (!data || len == 0) return true;
  DWORD written;
...
// ----------------------------------------------------------------------------
inline void safePrint(std::wstring_view wsv) {
  HANDLE h = getStdOut();
  if (isStdoutConsole()) {
...
inline void safeErrorPrint(std::wstring_view wsv) {
  HANDLE h = getStdErr();
  if (isStderrConsole()) {
    if (!detail::writeConsoleW(h, wsv.data(), wsv.size()) &&
        isBrokenPipeError(GetLastError())) {
...
inline void safePrintLn(std::wstring_view wsv) {
  safePrint(wsv);
  safePrint(L"\n");
}

inline void safeErrorPrintLn(std::wstring_view wsv) {
  safeErrorPrint(wsv);
  safeErrorPrint(L"\n");
}
...
// ----------------------------------------------------------------------------
inline void safePrint(std::string_view sv) {
  HANDLE h = getStdOut();
  if (isStdoutConsole()) {
...
inline void safeErrorPrint(std::string_view sv) {
  HANDLE h = getStdErr();
  if (isStderrConsole()) {
    detail::wchar_buffer buf(sv);
    if (buf.valid()) {
...
inline void safePrintLn(std::string_view sv) {
  safePrint(sv);
  safePrint("\n");
}

inline void safeErrorPrintLn(std::string_view sv) {
  safeErrorPrint(sv);
  safeErrorPrint("\n");
}
...
inline void safeErrorPrint(const char* str) {
...
inline void safeErrorPrintLn(const char* str) {
  if (str)
...
inline void safePrint(const wchar_t* wstr) {
  if (!wstr) return;
  safePrint(std::wstring_view(wstr));
}

inline void safeErrorPrint(const wchar_t* wstr) {
  if (!wstr) return;
  safeErrorPrint(std::wstring_view(wstr));
}

inline void safePrintLn(const wchar_t* wstr) {
  safePrint(wstr);
  safePrint(L"\n");
}

inline void safeErrorPrintLn(const wchar_t* wstr) {
  safeErrorPrint(wstr);
  safeErrorPrint(L"\n");
}
// ----------------------------------------------------------------------------
...
inline void safeErrorPrint(char c) {
  HANDLE h = getStdErr();
  if (isStderrConsole()) {
    wchar_t wc = static_cast<wchar_t>(c);
    detail::writeConsoleW(h, &wc, 1);
...
inline void safePrintLn(char c) {
  safePrint(c);
  safePrint("\n");
}

inline void safeErrorPrintLn(char c) {
  safeErrorPrint(c);
  safeErrorPrint("\n");
}
...
// ----------------------------------------------------------------------------
inline void safePrint(int n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%d", n);
  safePrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safeErrorPrint(int n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%d", n);
  safeErrorPrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safePrint(unsigned int n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%u", n);
  safePrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safeErrorPrint(unsigned int n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%u", n);
  safeErrorPrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safePrint(long n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%ld", n);
  safePrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safeErrorPrint(long n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%ld", n);
  safeErrorPrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safePrint(unsigned long n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%lu", n);
  safePrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safeErrorPrint(unsigned long n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%lu", n);
  safeErrorPrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safePrint(long long n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%lld", n);
  safePrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safeErrorPrint(long long n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%lld", n);
  safeErrorPrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safePrint(unsigned long long n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%llu", n);
  safePrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safeErrorPrint(unsigned long long n) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%llu", n);
  safeErrorPrint(std::string_view(buf, static_cast<size_t>(len)));
}
...
// Pointer overload
// ----------------------------------------------------------------------------
inline void safePrint(const void* ptr) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%p", ptr);
  safePrint(std::string_view(buf, static_cast<size_t>(len)));
}

inline void safeErrorPrint(const void* ptr) {
  char buf[32];
  int len = snprintf(buf, sizeof(buf), "%p", ptr);
  safeErrorPrint(std::string_view(buf, static_cast<size_t>(len)));
}
...
// Boolean overload
// ----------------------------------------------------------------------------
inline void safePrint(bool b) { safePrint(b ? "true" : "false"); }

inline void safeErrorPrint(bool b) { safeErrorPrint(b ? "true" : "false"); }
...
inline size_t string_display_width(const std::wstring& s) {
  size_t width = 0;
  for (wchar_t c : s) {
